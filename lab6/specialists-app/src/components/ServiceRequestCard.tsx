...
import { ISpecialist } from '../modules/get-specialist';
...
const ServiceRequestCard: FC<IServiceRequest> = ({id, creator, comment, status, specialist, created_at, formed_at, finished_at}) =>{
    ...
    const specialists_in_created: ISpecialist[] = useSelector((state: RootState) => state.specialists.specialists)
    ...
    const delete_request = async (id: Number) =>{
        ...
        console.log(specialists_in_created)
        dispatch(setNum(0))
        setDelCard(true)
            ...
            return (    
                <Card className="card">
                    <Card.Body>                
                        ...
                        <Row xs={4} md={4} className="g-4">
                            {specialists_in_created.map((item, index)=> (
                                <Col key={index}>
                                    {/* <a href={`/specialists/${item.id}`} style={{textDecoration: 'none'}}> */}
